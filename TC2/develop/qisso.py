...
#        continue
    loc +=1
    plt.subplot(ceil(n/(1.*columns)),columns,loc)
    plt.xlabel("MN#")
    plt.ylabel(params['graph_names'][key]+" ["+params['units'][key]+"]")
    paramplot = np.array(params[key])
    if any(np.array(['gnapbar','gksbar','gkfbar','gnabar','gl_soma'])==key):
        paramplot *= 1e3
    elif any(np.array(['gcaLbar','gl_caL'])==key):
        paramplot *= 1e6
    elif key=='L_dend':
        paramplot /= 1000
    else:
        pass
    plt.title(subid[loc-1],loc='left')
    #plt.plot(paramplot,'-',color='0.7')
    plt.plot(paramplot,'.',color=color,label=case)
